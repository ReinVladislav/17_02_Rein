...
public class GamePanel extends JFrame implements KeyListener{
    ...
    public GamePanel(GameModel model) {
        ...
        try
        {
            _cellImg = ImageIO.read(new File(folder + "Field/Earth.png"));
    ...
    */
    private BufferedImage getAmmoImage(AbstractAmmo ammo){
   ...
   @Override
    public void keyPressed(KeyEvent e)
    {
        ...
        if(_model.field()!= null && _model.getCurrentTank()!= null)
        {
                switch (e.getKeyCode()) {
                    case KeyEvent.VK_A:
                        _model.rotateCurrentTank(Rotation.Left());
                        break;
                    case KeyEvent.VK_D:
                        _model.rotateCurrentTank(Rotation.Right());
                        break;
                    case KeyEvent.VK_W:
                        _model.moveCurrentTank();
                        break;
                    case KeyEvent.VK_F:
                    {
                        //Если танк готов стрелять
                        if(_model.getCurrentTank().isReadyShoot() && _model.getCurrentTank().getCell().nextCell(_model.getCurrentTank().getDirection()) != null){
                            _model.shootCurrentTank();
                        }
                    }
                        break;
                    case KeyEvent.VK_SPACE:
                        _model.skipCurrentTank();
                        break;
                    case KeyEvent.VK_Q:
                    {
                        //Если танк готов стрелять
                        if(_model.getCurrentTank().isReadyShoot()){
                            ...
                            TurnableBullet ammo = new TurnableBullet(_model.getCurrentTank().getCell(), _model.getCurrentTank().getDirection(), 9);
